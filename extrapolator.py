...
class UnifiedExtrapolator(Extrapolator):
    def __init__(self, map_filename, map_generic_turn_probs_filename, map_trace_turn_probs_filename, classifier_path, mode="e", max_error_target=None):
        ...
        self.prev_location_params_time_limit = 60.0
    ...
    def _get_classifier(self, classifier_path, mode):
        ...
        if (mode == "e" or mode == "dx"):
            ...
            decision_tree_filenames = filter(lambda x: (x.startswith("oracle_tree_" + mode) or x.startswith("oracle_neural_network_" + mode)) and x.endswith(".pkl"), os.listdir(classifier_path))
            ...
            for decision_tree_filename in decision_tree_filenames:
                threshold = decision_tree_filename.split("_")[-1]
    ...
    def _get_feature_vector(self, curr_location, prev_locations):
        ...
        if (num_prev_locations > 0):
            ...
            prev_locations_mean_speed = 0.0
            curr_location_mean_speed_delta = 0.0
            curr_location_mean_acceleration_delta = 0.0
            curr_location_distance_to_mean_center = 0.0
            curr_location_distance_to_prev_location = 0.0
        
        return [curr_location_distance_to_map_location, prev_locations_mean_speed, curr_location_mean_speed_delta, curr_location_mean_acceleration_delta, curr_location_distance_to_mean_center, curr_location_distance_to_prev_location]
