...
class TimeErrorTableCreator:
    def __init__(self, window_size = 600, time_error_table_pickle_filename=None):
        ...
        if (time_error_table_pickle_filename is None):
            self.time_error_table = self.init_time_error_table() # NOTE: row 0 is for reachtime of interval 1 and so on...
        else:
...
if __name__ == "__main__":
    ...
    (opts, args) = getopt.getopt(sys.argv[1:],"i:o:x:w:e:f:p:g:c:t:h")
    ...
    timeErrorTableCreator = TimeErrorTableCreator(window_size, time_error_table_pickle_filename)
