...
class OracleTrainer:
    ...
    def train(self, trace, labeled_samples, error_mode_flag):
        all_trips_samples = self._get_all_trips_samples(trace)
        #print "all_trips_samples: " + str(all_trips_samples)
        
        all_labeled_samples = self._get_all_labeled_samples(labeled_samples, error_mode_flag)
        ...
        for curr_trip_samples in all_trips_samples:
            for i in range(0, len(curr_trip_samples)):
                ...
                # print class label
                sys.stdout.write(str(all_labeled_samples[labeled_sample_index][2]) + "\n")
                labeled_sample_index += 1
    
    def _get_all_labeled_samples(self, labeled_samples, error_mode_flag):
        all_labeled_samples = []
        
        for raw_sample in labeled_samples:
            sample_data, extrapolator_data = raw_sample.strip("\n").split(" x")
            
            _, lat, lon, _, _, _ = sample_data.strip("\n").split(" ")
            
            extrapolator_performance_pairs = map(lambda x: x.split(","), extrapolator_data.strip("\n").split(" ")[1:])
            
            if (error_mode_flag == 1):
                max_extrapolator_performance = max(map(lambda x: float(x[1]), extrapolator_performance_pairs))
                
                for i in range(0, len(extrapolator_performance_pairs)):
                    if (max_extrapolator_performance == 0.0):
                        extrapolator_performance_pairs[i][1] = 1.0
                    else:
                        extrapolator_performance_pairs[i][1] = (float(extrapolator_performance_pairs[i][1]) / max_extrapolator_performance)
            
            else:
                min_extrapolator_performance = min(map(lambda x: float(x[1]), extrapolator_performance_pairs))
                
                for i in range(0, len(extrapolator_performance_pairs)):
                    if (float(extrapolator_performance_pairs[i][1]) == 0.0):
                        extrapolator_performance_pairs[i][1] = 1.0
                    else:
                        extrapolator_performance_pairs[i][1] = (min_extrapolator_performance / float(extrapolator_performance_pairs[i][1]))
            
            label = " ".join(map(lambda x: str(x[0]) + "," + str(x[1]), extrapolator_performance_pairs))
...
if __name__ == "__main__":
    ...
    (opts, args) = getopt.getopt(sys.argv[1:],"t:l:m:e:h")
    ...
    for o,a in opts:
        if o == "-t":
            ...
            error_mode_flag = int(a)
        elif o == "-h":
            print "Usage: python oracle_trainer.py [-t <raw_trace_filename>] [-l <labeled_samples_filename>] [-m <map_filename>] [-e <error_mode_flag>] [-h]"
            exit()
    ...
    otr.train(raw_trace, labeled_samples, error_mode_flag)
