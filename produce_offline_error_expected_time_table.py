...
def get_expected_time_for_error(time_error_table, error):
...
def write_all_expected_times(time_error_table, output_file):
    if not time_error_table:
        return

    errors = sorted(list(set([error for (time, error) in time_error_table.keys()])))
    for e in errors:
        expected_time_e = get_expected_time_for_error(time_error_table, e)
        output_file.write(str(e)+" "+str(expected_time_e)+"\n")


def write_all_expected_times_pickle(time_error_table, output_file):
    if not time_error_table:
        return
...
usage="Usage: python produce_offline_error_expected_time_table -i <expected time error pickle file> -o <expected time data file> -p <expected time pickle file> [-h]"
...
if output_file != "":
    ...
    write_all_expected_times(expected_error_table, fo)
    fo.close()

if output_file_pickle != "":
    fp = open(output_file_pickle, "w")
    write_all_expected_times_pickle(expected_error_table, fp)
    fp.close()
