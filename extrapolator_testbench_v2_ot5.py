...
class ExtrapolatorTestbench:
    ...
    def _get_duration_errors(self, location_params, trip, extrapolator, max_duration, extrapolated_paths_file):
        ...
        for i in range(0, len(time_offsets)):
            ...
            duration_errors.append((curr_duration, curr_error, i))
    ...
    def _examine_duration_errors(self, duration_errors, error_thresholds, duration_thresholds):
        ...
        for error_threshold in error_thresholds:
            ...
            if (duration_errors_itr > 0):
                prev_duration, prev_error, prev_sample_index = duration_errors[duration_errors_itr - 1]
                error_threshold_samples[error_threshold] = (prev_duration, prev_error, prev_sample_index, int(crossed_error_threshold))
            else:
                error_threshold_samples[error_threshold] = (0.0, 0.0, 0, int(crossed_error_threshold))
        ...
        for duration_threshold in duration_thresholds:
            ...
            if (duration_errors_itr > 0):
                ...
                prev_sample_index = duration_errors[duration_errors_itr - 1][2]
                
                duration_threshold_samples['mean'][duration_threshold] = ((sum(prev_errors) / len(prev_errors)), prev_duration, prev_sample_index, int(crossed_duration_threshold))
                duration_threshold_samples['max'][duration_threshold] = (max(prev_errors), prev_duration, prev_sample_index, int(crossed_duration_threshold))
            else:
                duration_threshold_samples['mean'][duration_threshold] = (0.0, 0.0, 0, int(crossed_duration_threshold))
                duration_threshold_samples['max'][duration_threshold] = (0.0, 0.0, 0, int(crossed_duration_threshold))
    ...
    def _build_sample_time_table(self, trip, sample_index, extrapolator, error_thresholds, duration_thresholds, error_threshold_files, duration_threshold_files, max_duration, extrapolated_paths_file):
        error_threshold_output_count = [{x: 1 for x in error_thresholds} for sample in trip[:-1]]
        duration_threshold_output_count = [{x: 1 for x in duration_thresholds} for sample in trip[:-1]]
        ...
        for i in range(0, trip_length - 1):
            ...
            for error_threshold in error_thresholds:
                error_threshold_duration, _, error_threshold_trip_index, crossed_error_threshold = error_threshold_samples[error_threshold]
                error_threshold_files[error_threshold].write(str(sample_index) + " " + str(trip[i].lat) + " " + str(trip[i].lon) + " " + str(error_threshold_duration) + " " + str(crossed_error_threshold) + " " + str(error_threshold_output_count[i][error_threshold]) + "\n")
                
                if ((error_threshold_trip_index + i + 1) < (trip_length - 1)):
                    error_threshold_output_count[error_threshold_trip_index + i + 1][error_threshold] += error_threshold_output_count[i][error_threshold]
            
            for duration_threshold in duration_thresholds:
                duration_threshold_mean_error, _, _, crossed_duration_threshold = duration_threshold_samples['mean'][duration_threshold]
                duration_threshold_files['mean'][duration_threshold].write(str(sample_index) + " " + str(trip[i].lat) + " " + str(trip[i].lon) + " " + str(duration_threshold_mean_error) + " " + str(crossed_duration_threshold) + " " + str(duration_threshold_output_count[i][duration_threshold]) + "\n")
                
                duration_threshold_max_error, _, duration_threshold_trip_index, crossed_duration_threshold = duration_threshold_samples['max'][duration_threshold]
                duration_threshold_files['max'][duration_threshold].write(str(sample_index) + " " + str(trip[i].lat) + " " + str(trip[i].lon) + " " + str(duration_threshold_max_error) + " " + str(crossed_duration_threshold) + " " + str(duration_threshold_output_count[i][duration_threshold]) + "\n")
                
                if ((duration_threshold_trip_index + i + 1) < (trip_length - 1)):
                    duration_threshold_output_count[duration_threshold_trip_index + i + 1][duration_threshold] += duration_threshold_output_count[i][duration_threshold]
            
            sample_index += 1
        
        #for i in range(0, trip_length - 1):
        #    print "error_threshold_output_count[%d][1]: %s" % (i, error_threshold_output_count[i][1])
...
from extrapolator_ot5 import ConstantLocationExtrapolator, ConstantVelocityExtrapolator, ConstantAccelerationExtrapolator, ConstantDecelerationExtrapolator, MapExtrapolator, MapExtrapolatorStraightRoad, MapExtrapolatorNMM, MapExtrapolatorNMMStraightRoad, UnifiedExtrapolator
...
if __name__ == "__main__":
    ...
    elif (extrapolator_index == 28 or 
          extrapolator_index == 30 or 
          extrapolator_index == 32 or 
          extrapolator_index == 34 or 
          extrapolator_index == 36 or 
          extrapolator_index == 38 or 
          extrapolator_index == 40 or 
          extrapolator_index == 42):
    ...
    elif (extrapolator_index == 29 or 
          extrapolator_index == 31 or 
          extrapolator_index == 33 or 
          extrapolator_index == 35 or 
          extrapolator_index == 37 or 
          extrapolator_index == 39 or 
          extrapolator_index == 41 or 
          extrapolator_index == 43):
