...
class OracleTreePickler:
    ...
    def _get_features_and_classes(self, input_filename):
        ...
        for sample in fc_file:
            ...
            curr_features = map(lambda x: float(x), sample_components[:-1])
            curr_class = int(sample_components[-1])
            
            if (curr_class >= 0 and curr_class <= 3):
                remapped_class = 3
            
            elif (curr_class >= 4 and curr_class <= 11):
                remapped_class = 11
            
            features.append(curr_features)
            classes.append(remapped_class)
