...
class StreetMap:
    ...
    def load_osmdb(self, osmdb_filename, bbox_mode=None):
        ...
        for id, tags, nodes in query_result:
            ...
            if ('highway' in way_tags_dict and self._valid_highway_edge(way_tags_dict['highway'])):
                ...
                if (('oneway' not in way_tags_dict) or (way_tags_dict['oneway'].lower() == "no") or (way_tags_dict['oneway'].lower() == "false") or (way_tags_dict['oneway'].lower() == "0")):
    ...
    def _uic_bounding_box(self, node):
        ...
        # UIC_7M traces bounding box (+300 meter margin)
        if ((node.latitude >= 41.859246 and node.latitude <= 41.886777) and 
            (node.longitude >= -87.690391 and node.longitude <= -87.636272)):
    ...
    def _msmls_bounding_box(self, node):
        ...
        if ((node.latitude >= 46.639865 and node.latitude <= 48.617280) and 
            (node.longitude >= -123.168824 and node.longitude <= -121.437201)):
