...
class BudgetErrorTableCreator:
    ...
    def get_max_extrapolation_compression_error(self, extrapolation_errors, estimation_window_starting_compression_loc, isBegenning):
        ...
        try:
            ...
            while(curr_loc.time - delay_window_for_compression[0][0].time <= self.delay):
                if isBegenning:
                    compressed_window = self.get_compressed_window_size_bound(delay_window_for_compression, 4) 
                    compression_errors = self.get_compression_error(delay_window_for_compression, compressed_window)
                    max_error_in_extr_comp = max(extrapolation_errors + compression_errors)
                    time = delay_window_for_compression[-1][0].time
                    time_maxerror.append((time, max_error_in_extr_comp))
            ...
            #print "exception"
            time_maxerror.append((-1,-1))
            return time_maxerror
        ...
        return time_maxerror
        #return (time, max_error_in_extr_comp)
    ...
    def update_time_error_table(self, curr_loc_param, extrapolator):
        ...
        if(curr_loc.time - self.estimation_window[0][0].time <= self.window_size):
            ...
            for i in range(0, len(extrapolation_errors)):
                ...
                if i is 0:  
                    time_maxerror = self.get_max_extrapolation_compression_error(extrapolation_errors[0:i], self.estimation_window[i:], True)
                else:
                    time_maxerror = self.get_max_extrapolation_compression_error(extrapolation_errors[0:i], self.estimation_window[i:], False)

                for i in range(len(time_maxerror)):
                    max_extr_comp_error_at_time = time_maxerror[i]
                    if max_extr_comp_error_at_time[0] == -1:
                        break


                    if debug: print max_extr_comp_error_at_time, max_extr_comp_error_at_time[0]-start_loc.time
                    (window_end_time, max_extr_comp_error) = max_extr_comp_error_at_time
                    error_bucket = self.get_bucket_from_error(max_extr_comp_error)
...
if __name__ == "__main__":
    ...
    (opts, args) = getopt.getopt(sys.argv[1:],"i:o:x:w:d:e:f:p:g:c:t:h")
    ...
    for o,a in opts:
        if o == "-i":
            ...
            print "Usage: python produce_offline_time_error_table [-i input_file_name] [-o output_file_name] [-x extrapolator_index] [-w window_size] [-d delay] [-e <sampler_error_threshold>] [-f <map_filename>] [-p <map_trace_turn_probs_filename>] [-g map_generic_turn_probs_filename] [-c <classifier_path>] [-t <time_error_table_pickle_filename>] [-h]"
            exit()
    
    if input_filename == "" or output_filename=="":
        print "Usage: python produce_offline_reachtime_exponentialerror_table [-i input_file_name] [-o output_file_name] [-x extrapolator_index] [-w window_size] [-d delay] [-e <sampler_error_threshold>] [-f <map_filename>] [-p <map_trace_turn_probs_filename>] [-g map_generic_turn_probs_filename] [-c <classifier_path>] [-t <time_error_table_pickle_filename>] [-h]"
        exit()
