...
for file in `ls budget_exponentialerror_table_offline/*.pkl`; do
  file_no_prefix=${file##*/}
  echo "python produce_offline_exponentialerror_expected_budget_table.py -i budget_exponentialerror_table_offline/$file_no_prefix -o ../data/expected_budget_for_exponentialerror_${file_no_prefix%.pkl}.txt -p expected_budget_for_exponentialerror_offline/$file_no_prefix"
done
