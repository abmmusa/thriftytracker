...
def get_expected_budget_for_errorbucket(time_error_table, errorbucket):
...
def write_all_expected_budgets(time_error_table, output_file):
    if not time_error_table:
        return

    for bucket in range(0, MAX_BUCKETS):
        expected_time_bucket = get_expected_budget_for_errorbucket(time_error_table, bucket)
        output_file.write(str(bucket)+" "+str(expected_time_bucket)+"\n")


def write_all_expected_budgets_pickle(time_error_table, output_file):
    if not time_error_table:
        return
...
if output_file != "":
    ...
    write_all_expected_budgets(error_budget_table, fo)
    fo.close()

if output_file_pickle != "":
    fp = open(output_file_pickle, "w")
    write_all_expected_budgets_pickle(error_budget_table, fp)
    fp.close()
