...
def write_all_expected_budgets_wo_hump(table, output_file):
    if not table:
        return 
...
def write_all_expected_budgets_pickle_wo_hump(table, output_file):
    if not table:
        return
...
usage="Usage: python produce_offline_exponentialerror_expected_budget_table_wo_hump.py -i <expected budget exponentialerror pickle file> -o <expected budget wo hump data file> -p <expected budget wo hump pickle file> [-h]"
...
expected_exponential_budget_table = pickle.load(f)
f.close()


if output_file != "":
    fo = open(output_file, "w")
    write_all_expected_budgets_wo_hump(expected_exponential_budget_table, fo)
    fo.close()

if output_file_pickle != "":
    fp = open(output_file_pickle, "w")
    write_all_expected_budgets_pickle_wo_hump(expected_exponential_budget_table, fp)
    fp.close()
